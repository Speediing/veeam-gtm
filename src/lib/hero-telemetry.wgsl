...
fn veeamMark(p: vec2f) -> f32 {
  let q = rotate(p, -0.08);
  let badge = sdRoundedBox(q, vec2f(0.22, 0.16), 0.045);
  let inset = sdRoundedBox(q, vec2f(0.14, 0.075), 0.018);
  let stem = sdRoundedBox(q - vec2f(0.0, 0.085), vec2f(0.028, 0.055), 0.012);
  return opSmoothSub(opSmoothUnion(badge, stem, 0.02), inset, 0.012);
}
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let veeamGreen = vec3f(0.0, 0.819608, 0.372549);
  ...
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(veeamGreen, paper, spark * 0.45 + line * 0.05);
